...
const (
	BINANCE_BASE_URL = "https://api.huobi.pro"
)
...
func (h *BinanceApi) TradeFeeRate() (map[string]map[string]TradeFee, error) {
...
func (h *BinanceApi) TransferFee() (map[string]float64, error) {
	...
	ch := make(chan *BinanceTransferFeeResponse, len(currencies))
	...
	for _, c := range currencies {
		...
		go func(currency string) {
			...
			if err != nil {
				ch <- &BinanceTransferFeeResponse{nil, currency, err}
				return
			}
			defer resp.Body.Close()
			byteArray, err := ioutil.ReadAll(resp.Body)
			ch <- &BinanceTransferFeeResponse{byteArray, currency, err}
			<-workers
		}(c)
	}
	go func() {
...
func (h *BinanceApi) getAccountId() (string, error) {
...
func (h *BinanceApi) CancelOrder(orderNumber string, _ string) error {
	params := &url.Values{}
...
func (h *BinanceApi) IsOrderFilled(orderNumber string, _ string) (bool, error) {
	params := &url.Values{}
	params.Set("order-id", orderNumber)
